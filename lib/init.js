				...
				if (!user) {
                    ...
                    if (user.schema.path('email')) {
                        if(parsedProfile.email) {
                            ...
                            // its mandatory these days - we need a value here...
                            user.set('email', parsedProfile.id + '@' + provider.name)
                        }
                    }
                    
                    if(user.schema.path('password')) {
                        user.set('password', parsedProfile.id)
                    }
					...
					if (user.schema.virtual('name.full') && parsedProfile.name) {
						user.set('name.full', provider.name);
					}
